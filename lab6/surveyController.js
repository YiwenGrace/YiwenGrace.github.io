...
module.exports = function(app){
    ...
    app.get('/analysis', function(req, res){
        var info = readData("info");
        var freq = readData("freq");
        var fname = readData("fname");
        var fm = readData("fm");
        var likert = readData("likert");
        var comment = readData("comment");
        res.render('showResults', {results: [fname, fm, freq, info, comment, likert]});
        console.log([fname, fm, freq, info, comment, likert]);
    });
    ...
    app.post('/index', urlencodedParser, function(req, res){
        ...
        for (var key in json){
            ...
            if ((key === "info") && (json[key].length === 2)){
                for (var item in json[key]){
                    combineCounts(key, json[key][item]);
                }
            }
            else if ((key === "info") && (json[key].length === 3)){
              for (var item in json[key]){
                  combineCounts(key, json[key][item]);
              }
            }
            else if ((key === "info") && (json[key].length === 4)){
              for (var item in json[key]){
                  combineCounts(key, json[key][item]);
              }
            }
